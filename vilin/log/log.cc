...
void Logger::setFormatter(const std::string& val) {
	vilin::LogFormatter::ptr new_val(new vilin::LogFormatter(val));
	if(new_val->isError()) {
...
vilin::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = vilin::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
	LogIniter() {
		g_log_defines->addListener(0xF1E231, [](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value) {
			VILIN_LOG_INFO(VILIN_LOG_ROOT()) << "on_logger_conf_changed";
			for(auto &i : new_value) {
				auto it = old_value.find(i);
				vilin::Logger::ptr logger;
				if(it == old_value.end()) {
					logger = VILIN_LOG_NAME(i.name);
				} else {
					if(!(i == *it)) {
						//修改
						logger = VILIN_LOG_NAME(i.name);
			...
			for(auto &i : old_value) {
				...
				if(it == new_value.end()) {
					//删除
					auto logger = VILIN_LOG_NAME(i.name);
